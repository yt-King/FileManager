...
public class FileViewAdapter extends RecyclerView.Adapter<FileViewAdapter.ViewHolder> implements ItemTouchCallBack.OnItemTouchListener {
  ...
  private final List<FileView> fileList;  //文件列表
  ...
  public FileViewAdapter(List<FileView> fileList) {
    ...
    fileTypeIconMap.put("doc", R.drawable.filetype_doc);
    fileTypeIconMap.put("xls", R.drawable.filetype_excel);
  }
    ...
    holder.fileView.setOnClickListener(v -> {
      ...
      if (file.isFolder()) {
        ...
        try {
          ...
          //根据文件类型创建intent打开
          Intent intent = OpenFileUtils.openFile(file.getFile().toString());
